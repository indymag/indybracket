...
public class Grader {
  ...
  public BracketResult grade(Bracket oSubject) {
    ...
    long score = gradeTree(moMaster.getChampionship(),
        oResult.getChampionship());
  ...
  public long findMax(Bracket oSubject) {
    return gradeTree(getBest(moMaster, oSubject).getChampionship(),
        oSubject.getChampionship());
  }
  ...
  private boolean canWin(Match oMaster, Match oSubject) {
    ...
    // if a winner has been set for the master, return if true if there is a
    // match
    if (oMaster.getWinner() != null) {
      return oMaster.getWinner() == oSubjectWinner;
    }

    // if the master is unset and is first round match, of course the team has a
    // chance
